...
APP_TITLE = "Marvin Webhook Receiver"
logger = logging.getLogger("marvin_webhook")
logging.basicConfig(level=logging.INFO)

BASE_API_URL = os.environ.get("OPENDESK_API_BASE_URL", "http://localhost:8000/api/v1")
SECRET = os.environ.get("MARVIN_WEBHOOK_SECRET", "")
...
def _verify_signature(body_bytes: bytes, signature_header: Optional[str]) -> bool:
    """Verify HMAC signature from Marvin webhook."""
    if not SECRET:
        # SECURITY: Fail closed when no secret is configured.
        # In production, MARVIN_WEBHOOK_SECRET must always be set.
        logger.error(
            "MARVIN_WEBHOOK_SECRET is not configured; "
            "webhook signature verification is disabled. "
            "Set MARVIN_WEBHOOK_SECRET environment variable."
        )
        return False
...
async def _forward_to_api(
    ...
    """Forward webhook payload to internal LMS API."""
...
@app.post("/api/v1/webhooks/marvin")
async def handle_webhook(request: Request):
    """Handle Marvin webhook events."""
    body = await request.body()
    signature = request.headers.get("X-MARVIN-SIGNATURE") or request.headers.get(
        "X-Marvin_Signature"
    )
    ...
    # Map Marvin events to internal LMS API endpoints
    ...
    elif event_type == "student.created":
        await _forward_to_api("/students", "POST", data)
    elif event_type == "student.deleted":
        student_id = data.get("id") or data.get("student_id")
        if student_id:
            await _forward_to_api(f"/students/{student_id}", "DELETE")
    else:
        logger.info(f"Unhandled Marvin event: {event_type}")
...
if __name__ == "__main__":
    ...
    port = int(os.environ.get("MARVIN_WEBHOOK_PORT", "8002"))
