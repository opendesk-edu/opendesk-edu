...
class RandomUser:
    def __init__(
        ...
        amount=100,
        password_reset_mail="not_provided@opendesk.internal",
        ...
        self.input_files_list = {
            "firstname": os.path.join(
                os.path.dirname(__file__), "..", "data", "firstname_gender.tsv"
            ),
            "lastname": os.path.join(
                os.path.dirname(__file__), "..", "data", "lastname.txt"
            ),
            "organisation": os.path.join(
                os.path.dirname(__file__), "..", "data", "organisation.txt"
            ),
            "city": os.path.join(os.path.dirname(__file__), "..", "data", "city.txt"),
            "postcode": os.path.join(
                os.path.dirname(__file__), "..", "data", "postcode.txt"
            ),
            "street": os.path.join(
                os.path.dirname(__file__), "..", "data", "street.txt"
            ),
            "phone": os.path.join(os.path.dirname(__file__), "..", "data", "phone.txt"),
            "mobileTelephoneNumber": os.path.join(
                os.path.dirname(__file__), "..", "data", "mobile.txt"
            ),
        }
        self.lists = {}
        for _ in list(range(amount)):
            ...
            person["username"] = self.__get_username(
            ...
            if create_admin_accounts:
                person["username"] = self.__get_username(
                    person["firstname"], person["lastname"], admin=True
                )
    ...
    def __get_random_list_entry(self, category):
        if not category in self.lists:
            ...
            self.lists[category] = [
                entry for entry in lines_with_comments if not entry.startswith("#")
            ]
        return random.choice(self.lists[category])

    def __get_username(self, firstname, lastname, admin=False):
        if self.randomize_username:
            username = unicodedata.normalize("NFKD", firstname + "." + lastname).encode(
                "ascii", "ignore"
            )
            if admin:
